...
__DEFAULT_PREFS = {
    ...
    "email_messages": {},
    # general:
    #   show_log_in_gui  — existing, controls the log panel
    #   max_concurrent_feeds — v9, how many RSS feeds may be fetched in parallel.
    #     Default 1 preserves previous behaviour (one-at-a-time) for safety.
    "general": {"show_log_in_gui": True, "max_concurrent_feeds": 1},
}
...
class YARSSConfig(object):
    ...
    def update_config_to_version9(self, config):
        """Updates the config to version 9 (YaRSS2 v2.2.0).

        Adds per-feed fields:
          * verify_tls       — TLS certificate verification toggle (default True)
          * etag             — HTTP ETag cache validator (internal)
          * last_modified    — HTTP Last-Modified cache validator (internal)

        Adds to general config:
          * max_concurrent_feeds — parallelism for RSS feed fetches (default 1)
        """
        self.log.info("Updating config file to version 9")
        default_rssfeed_config = get_fresh_rssfeed_config()

        def update_rssfeed(rssfeed):
            rssfeed.setdefault("verify_tls", default_rssfeed_config["verify_tls"])
            rssfeed.setdefault("etag", default_rssfeed_config["etag"])
            rssfeed.setdefault("last_modified", default_rssfeed_config["last_modified"])

        self.run_for_each_dict_element(config["rssfeeds"], update_rssfeed)

        general = config.get("general", {})
        general.setdefault("max_concurrent_feeds", 1)
...
def get_fresh_rssfeed_config(name=u"", url=u"", site=u"", active=True, last_update=u"",
    ...
    # v9 additions:
    # verify_tls — whether to verify the TLS cert chain when downloading torrents from this feed.
    # Default True (secure). Users with self-signed private trackers can opt out.
    config_dict["verify_tls"] = True
    # etag / last_modified — HTTP cache validators. Populated after a successful fetch and
    # sent back on subsequent requests as If-None-Match / If-Modified-Since to avoid
    # re-downloading unchanged feeds. Internal; not normally edited by the user.
    config_dict["etag"] = u""
    config_dict["last_modified"] = u""
    if key:
        config_dict["key"] = key
    return config_dict
